...
contract Lottery is OwnableUpgradeable {
    ...
    function buy(uint256 _price, uint8[4] memory _numbers) external {
        ...
        cfuture.safeTransferFrom(address(msg.sender), address(this), _price);
    ...
    function multiBuy(uint256 _price, uint8[4][] memory _numbers) external {
        ...
        cfuture.safeTransferFrom(address(msg.sender), address(this), totalPrice);
    ...
    function claimReward(uint256 _tokenId) external {
        ...
        if(reward>0) {
            safecfutureTransfer(address(msg.sender), reward);
        }
    ...
    function multiClaim(uint256[] memory _tickets) external {
        ...
        if(totalReward>0) {
            safecfutureTransfer(address(msg.sender), totalReward);
        }
    ...
    // Safe cfuture transfer function, just in case if rounding error causes pool to not have enough cfutures.
    ...
    function adminWithdraw(uint256 _amount) external onlyAdmin {
        cfuture.safeTransfer(address(msg.sender), _amount);
