...
// added functionnalities by the CFuture's devs
//
...
contract TheBushV1 is Ownable {
    ...
    // The block number when CFT mining ends.
    ...
    function pendingReward(address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 cfutureReward = multiplier.mul(rewardPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
    ...
    // Withdraw CFT tokens from STAKING.
